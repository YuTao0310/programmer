...
[volatile修饰引用变量的问题](https://www.jianshu.com/p/10ef84dc5eef) 
[Volatile对于引用对象的修饰](https://blog.csdn.net/linsir20/article/details/105321557)
[stackoverflow相关问题](https://stackoverflow.com/questions/53753792/java-volatile-array-my-test-results-do-not-match-the-expectations)
...
https://www.yuque.com/snailclimb/mf2z3k/ks9olb19hc9wse5k（cnk4）
...
2、使用Netty提供的CompositeByteBuf，可以将多个ByteBuf合并为一个逻辑上的ByteBuf，避免各个ByteBuf之间的拷贝
3、通过Unpooled.wrappedBuffer可以将byte数组包装成ByteBuf对象，包装过程不会产生内存拷贝。
4、ByteBuf支持slice操作，因此可以将ByteBuf分解为多个共享同一个·存储区域的ByteBuf，避免内存的拷贝。
5、通过FileRegion包装的FileChannel#transferTo()实现文件传输，可以直接将文件缓存区的数据发送到目标channel，避免传统通过循环write方式导致的内存拷贝问题。

## 操作系统如何保证生成唯一的进程id

进程池，维护一个未被使用的进程list，一旦需要生成id则从这里面取，如果不需要则将id回收进该池子。
...
使用fork函数得到的子进程从父进程的继承了整个进程的地址空间和页表（拷贝相应的结构体），包括：进程上下文、进程堆栈、内存信息、打开的文件描述符、信号控制设置、进程优先级、进程组号、当前工作目录、根目录、资源限制、控制终端等。 
...
https://blog.csdn.net/ZXin153/article/details/111473283
...
通常在CPU 第一次轮询时就可以返回结果。此时如果使用中断，反而会使系统变慢：切换到
...
> 提示：中断并非总是比PIO （Pooling IO）好。尽管中断可以做到计算与I/O 的重叠，但这仅在慢速设备上有意义。否则，额外的中断处理和上下文切换的代价反而会超过其收益。另外，如果短时间内出现大量的中断，可能会使得系统过载并且引发活锁[。这种情况下，轮询的方式可以在操作系统自身的调度上提供更多的控制，反而更有效。
...
数据链路层互联的设备是网桥 (bridge),在网络互联中它起到**数据接收、地址过滤与数据转发的作用**，用来实现多个网络系统之间的数据交换。
...
https://xiaolincoding.com/network/1_base/tcp_ip_model.html#%E5%BA%94%E7%94%A8%E5%B1%82
...
可以从B/S C/S架构 服务发现 底层连接形式 （有无使用连接池）传输内容考虑（序列化协议；字段值冗余程度）

# SpringBoot

## Spring源码分析

https://www.yuque.com/snailclimb/gepg7u/cgt8a0
...
原文链接：https://blog.csdn.net/weixin_41954254/article/details/106414746
